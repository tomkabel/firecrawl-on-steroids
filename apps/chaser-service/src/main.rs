...
async fn handle_scrape(
    ...
    let (page_status_code, page_error) = match &nav_result {
        ...
        Ok(Err(e)) => {
            ...
            // Chrome net errors (e.g. net::ERR_NAME_NOT_RESOLVED) are not HTTP
            // status codes, so we cannot derive a meaningful status from them.
            // The previous extraction branch always resolved to 0, so we keep it
            // flat instead of carrying dead, misleading logic.
            (0, Some(err_str))
        }
        Err(_elapsed) => {
            // Timeout
    ...
    if let Some(ref selector) = req.check_selector {
        if page_status_code == 200 {
            // Encode the client-supplied selector as a JSON string literal so it
            // cannot break out of the JS string (the previous manual escaping of
            // single quotes left a trailing-backslash injection vector).
            let selector_literal = serde_json::to_string(selector)
                .unwrap_or_else(|_| "null".to_string());
            let check_result = chaser
                .evaluate(&format!("!!document.querySelector({})", selector_literal))
                .await;
