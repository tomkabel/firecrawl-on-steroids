...
# RabbitMQ Erlang cookie — generate a strong random one if not supplied so we
# never fall back to the well-known in-repo default in production.
RABBITMQ_ERLANG_COOKIE="${RABBITMQ_ERLANG_COOKIE:-$(openssl rand -hex 16 2>/dev/null || echo firecrawl-cookie-secret)}"
...
# Pre-compute Obscura wiring BEFORE starting Playwright so the Playwright
# container receives the correct OBSCURA_CDP_URL / RENDER_BACKEND_ORDER when
# START_OBSCURA is enabled. (Obscura itself connects lazily at scrape time, so
# its container does not need to exist yet.)
...
# OBSCURA_CDP_URL / RENDER_BACKEND_ORDER are already computed above so that
# Playwright (started earlier) receives them.
if [ "${START_OBSCURA:-false}" = "true" ]; then
...
# Requires STEALTH_BROWSER_MCP_URL pointing to a running stealth-browser-mcp instance (--transport http).
