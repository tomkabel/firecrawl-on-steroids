...
mkdir -p output
# Emit one lossless base64-encoded record per page, then decode inside the loop.
# Never split rows on tabs/newlines: real markdown contains both, so a tab-split
# (or `@tsv`, which escapes them to literal \t and \n) mangles page content.
jq -r '.data[] | @base64' output/crawl-results.json | while read -r row; do
  doc=$(printf '%s' "$row" | base64 --decode)
  url=$(printf '%s' "$doc" | jq -r '.metadata.sourceURL')
  md=$(printf '%s' "$doc" | jq -r '.markdown // ""')
  filename=$(printf '%s' "$url" | sed 's|https\?://||; s|/$||; s|/|_|g').md
  printf '%s' "$md" > "output/$filename"
...
firecrawl --api-url http://localhost:3002 map https://example.com --json \
  | jq -r '.links[].url' \
  | katana -silent -f url \
  | xargs -P 4 -I{} sh -c 'name=$(echo "$1" | unfurl format %p-%f); firecrawl --api-url http://localhost:3002 "$1" --only-main-content -o "output/$name.md"' sh {}
```
...
cat output/ffuf.json | jq -r '.results[].url' \
  | xargs -P 8 -I{} sh -c 'name=$(echo "$1" | unfurl format %p); firecrawl --api-url http://localhost:3002 "$1" -o "output/$name.md"' sh {}
```
