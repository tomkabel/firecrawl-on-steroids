...
// Fail closed: OBSCURA_CDP_SECRET is mandatory. Absence is a hard startup
// error (see the `:?err` guard in docker-compose.obscura.yaml and the
// START_OBSCURA check in start-firecrawl.sh), never a silent "unauthenticated"
// fallback. `.expect` makes a missing secret abort the process, so obscura is
// never exposed without auth on the Docker network.
let expected = std::env::var("OBSCURA_CDP_SECRET")
    .expect("OBSCURA_CDP_SECRET is required; obscura refuses to start without CDP auth");

// Every request must present the exact secret. With `expected` guaranteed
// non-empty above, there is no empty-string bypass path that silently allows
// unauthenticated access.
if query_params.get("cdp-secret") != Some(&expected) {
...
**Fail-closed auth (no silent allow):** `OBSCURA_CDP_SECRET` is required, not optional. The `:?err` guard in `docker-compose.obscura.yaml` and the `START_OBSCURA` check in `start-firecrawl.sh` already refuse to launch obscura without it, and the server above now `.expect()`s the variable at boot. A missing secret therefore fails the process at startup — obscura is never left unauthenticated on the Docker network, and there is no empty-string bypass that permits connections without the secret.
...
async function connectObscura(): Promise<Browser> {
  // Fail closed on missing CDP credentials. A missing URL *or* a missing secret
  // disables the obscura backend entirely: we throw so the caller skips to the
  // next backend in RENDER_BACKEND_ORDER. We never silently downgrade to an
  // unauthenticated CDP connection — absence of the secret is a hard error,
  // matching the `.expect()` the obscura server does at startup (Step 1.2).
  if (!OBSCURA_CDP_URL) {
    throw new Error('OBSCURA_CDP_URL not set — obscura backend disabled');
  }
  if (!OBSCURA_CDP_SECRET) {
    throw new Error(
      'OBSCURA_CDP_SECRET not set — obscura backend disabled (refusing to connect without CDP auth)',
    );
  }
  const wsUrl = `${OBSCURA_CDP_URL}?cdp-secret=${encodeURIComponent(OBSCURA_CDP_SECRET)}`;
...
async function getBrowserForBackend(backend: RenderBackend): Promise<Browser> {
  if (backend === RenderBackend.OBSCURA) {
    if (!obscuraBrowser) obscuraBrowser = await connectObscura();
    return obscuraBrowser;
  }
...
app.post('/scrape', async (req, res) => {
  ...
  // Walk RENDER_BACKEND_ORDER exactly once. Each iteration targets a *different*
  // backend via getBrowserForBackend(backend), so a scrape failure advances to
  // the next backend instead of re-running the same selection logic.
  for (const backend of RENDER_BACKEND_ORDER) {
    try {
      const browser = await getBrowserForBackend(backend);
      const result = await scrapePage(browser, url, timeout, wait_after_load, headers, skip_tls_verification);
      res.json({ ...result, _backend: backend }); // _backend is stripped in production, useful for debugging
      return;
    } catch (err) {
      console.warn(`Scrape failed on ${backend}, advancing to next backend:`, (err as Error).message);
      continue;
    }
  }
...
- **Fail closed on missing CDP credentials**: if `OBSCURA_CDP_URL` is set but `OBSCURA_CDP_SECRET` is not, `connectObscura()` throws and the obscura backend is treated as **disabled** — the request advances to the next backend. A missing secret is never treated as "connect without auth"
- On scrape failure the loop **advances to the next backend** in `RENDER_BACKEND_ORDER` — each attempt targets a different backend, so it never re-runs the same failed selection
...
# Shared SSRF guard — mirrors @firecrawl/ssrf-protection (Phase 1.0). Every
# adapter validates the target URL before navigating; no adapter is exempt.
from ssrf import assert_safe_target_url, InsecureConnectionError
...
async def scrape(req: ScrapeRequest):
    ...
    # SSRF gate: validate the target URL *before* any browser navigation so
    # private/internal addresses are blocked exactly like every other adapter.
    # Return the canonical blocked response shape (HTTP 200, pageStatusCode 403).
    try:
        assert_safe_target_url(req.url)
    ...
    # Increment the in-flight counter first, then guarantee a matching decrement
    # in the outer `finally` on *every* path — including get_browser() raising on
    # an exhausted pool. Pool acquisition stays inside this try/finally so the
    # health counters can never drift under load.
    active_requests += 1
    try:
        browser = get_browser()
        try:
            ...
            # Reached only after a browser was acquired, so it is always returned.
            return_browser(browser)
    except RuntimeError:
        # get_browser() found the pool exhausted. The outer `finally` below still
        # runs and decrements active_requests, so the counter never leaks.
        raise HTTPException(status_code=503, detail="browser pool exhausted")
    finally:
        # Always runs — whether the scrape succeeded, failed, or the pool was
        # exhausted — keeping active_requests balanced.
        active_requests -= 1
```
